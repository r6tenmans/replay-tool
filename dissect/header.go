...
type Header struct {
	...
	PlaylistCategory       int64     `json:"playlistCategory,omitempty"`
...
type Player struct {
	...
	InitialOperator Operator `json:"initialOperator,omitempty"`
...
func (r *Reader) readHeader() (Header, error) {
	...
	for lastProp := false; !lastProp; {
		...
		if !playerData {
			if k == "gmsetting" {
				gm, err := strconv.ParseInt(v, 10, 64)
				if err != nil {
					return Header{}, err
				}
				gmSettings = append(gmSettings, gm)
			} else {
				props[k] = v
			}
		} else {
	...
	// Parse map
	worldID, err := strconv.ParseInt(props["worldid"], 10, 64)
	if err != nil {
		return h, err
	}
	h.Map = Map(worldID)
	...
	// Parse game mode
	gameModeID, err := strconv.ParseInt(props["gamemodeid"], 10, 64)
	if err != nil {
		return h, err
	}
	h.GameMode = GameMode(gameModeID)
	...
	if len(props["playlistcategory"]) > 0 {
		n64, parseErr := strconv.ParseInt(props["playlistcategory"], 10, 64)
		if parseErr != nil {
			log.Debug().Err(parseErr).Msg("omitting playlistcategory")
		}
		h.PlaylistCategory = n64
	}
	// Add match id
	h.MatchID = props["id"]
	// Parse team scores
