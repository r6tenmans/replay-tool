...
func (m *MatchReader) Data() any {
	...
	for _, r := range m.rounds {
		rounds = append(rounds, round{
			...
			PositionUpdates: r.PositionUpdates,
			AmmoUpdates:     r.AmmoUpdates,
			Loadouts:        r.Loadouts,
			TrackedEntities: r.TrackedEntities,
			ShotEvents:      r.ShotEvents,
			GameActions:     r.GameActions,
			ScoreUpdates:    r.ScoreUpdates,
			DeathTimings:    r.DeathTimings,
			CameraFrames:    r.CameraFrames,
			HealthUpdates:   r.HealthUpdates,
			TimerTicks:      r.TimerTicks,
			TimerPhases:     r.TimerPhases,
			RecordingPlayer: r.RecordingPlayer,
		})
	}
	return output{
		Rounds:      rounds,
