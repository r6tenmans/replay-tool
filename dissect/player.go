...
func readAtkOpSwap(r *Reader) error {
	// Y10S4+: swap detection via readAtkOpSwap is unreliable (non-unique uiIDs,
	// wrong skip distance). Swaps are handled by reconcileOperatorSwaps using
	// the header's RoleName which already reflects the final operator.
